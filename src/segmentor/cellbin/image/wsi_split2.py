...
class SplitWSI(object):
    def __init__(self, img, win_shape, overlap=0, batch_size=0,
                 need_fun_ret=False, need_combine_ret=True, editable=False, tar_dtype=np.uint8, func_name='',dst_shape=None):
        """
    ...
    def _f_run(self):
        ...
        for i in tqdm(range(0, len(self._box_lst), self._batch_size),  file=clog.tqdm_out, mininterval=10, desc=self._func_name):
